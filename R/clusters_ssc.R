...
nclust_vs_BIC <- ggplot(tibble(x = 1:20, BIC = BICs), aes(x = x, y = BICs)) +
  ...
  ylab("mBIC")
ggsave(filename = "pictures/nclust_vs_BIC.png", nclust_vs_BIC)
...
ggsave(filename = "pictures/map_pm_vcl.png", map_pm_vcl, scale = 1.35)
...
map_pm_ssc <- draw.map(every_cl[[10]][1:263], clusters = c(1:2, 5:7, 9:10)) +
  ggtitle("Particle matter measurements clustered using varclust") + 
  theme(plot.title = element_text(hjust=0.5))
ggsave(filename = "pictures/map_pm_ssc.png", map_pm_ssc, scale = 1.35)
...
save(stability_mbic, file = "data/stability_mbic.rda")
...
ggsave("pictures/mbic_plot.png", mbic_plot)
